...
def multiple_gaussian(x, centers, sigmas, amplitudes):
    ...
    """
    if not len(centers) == len(sigmas) == len(amplitudes):
        raise ValueError("centers, sigmas, amplitudes must have same length")
    return sum(
        amp * np.exp(-((x - c) ** 2) / (2 * sig**2)) for c, sig, amp in zip(centers, sigmas, amplitudes, strict=False)
    )
...
class Gaussian(FitModelBase):
    ...
    def estimate_peak(self, data, x):
        ...
        # Smooth data
        filter_width = max(1, round(len(x) / 20))
    ...
    def estimate_dip(self, data, x):
        ...
        estimate["offset"].set(
            value=-estimate["offset"].value, min=-estimate["offset"].max, max=-estimate["offset"].min
        )
        estimate["amplitude"].set(
            value=-estimate["amplitude"].value, min=-estimate["amplitude"].max, max=-estimate["amplitude"].min
        )
        return estimate
...
class DoubleGaussian(FitModelBase):
    ...
    def _model_function(x, offset, center_1, center_2, sigma_1, sigma_2, amplitude_1, amplitude_2):
        return offset + multiple_gaussian(x, (center_1, center_2), (sigma_1, sigma_2), (amplitude_1, amplitude_2))

    @estimator("Peaks")
    def estimate_peaks(self, data, x):
        data, x = sort_check_data(data, x)
        data_smoothed, filter_width = smooth_data(data)
        leveled_data_smooth, _offset = correct_offset_histogram(data_smoothed, bin_width=2 * filter_width)
    ...
    def estimate_dips(self, data, x):
        ...
        estimate["amplitude_1"].set(
            value=-estimate["amplitude_1"].value, min=-estimate["amplitude_1"].max, max=-estimate["amplitude_1"].min
        )
        estimate["amplitude_2"].set(
            value=-estimate["amplitude_2"].value, min=-estimate["amplitude_2"].max, max=-estimate["amplitude_2"].min
        )
        return estimate
...
class TripleGaussian(FitModelBase):
    ...
    @staticmethod
    def _model_function(
        x, offset, center_1, center_2, center_3, sigma_1, sigma_2, sigma_3, amplitude_1, amplitude_2, amplitude_3
    ):
        return offset + multiple_gaussian(
            x, (center_1, center_2, center_3), (sigma_1, sigma_2, sigma_3), (amplitude_1, amplitude_2, amplitude_3)
        )
    ...
    def estimate_peaks(self, data, x):
        ...
        params["amplitude_1"].set(value=estimate["height"][0], min=limits["height"][0][0], max=limits["height"][0][1])
        params["amplitude_2"].set(value=estimate["height"][1], min=limits["height"][1][0], max=limits["height"][1][1])
        params["amplitude_3"].set(value=estimate["height"][2], min=limits["height"][2][0], max=limits["height"][2][1])
        params["center_1"].set(value=estimate["center"][0], min=limits["center"][0][0], max=limits["center"][0][1])
        params["center_2"].set(value=estimate["center"][1], min=limits["center"][1][0], max=limits["center"][1][1])
        params["center_3"].set(value=estimate["center"][2], min=limits["center"][2][0], max=limits["center"][2][1])
        params["sigma_1"].set(
            value=estimate["fwhm"][0] / 2.3548, min=limits["fwhm"][0][0] / 2.3548, max=limits["fwhm"][0][1] / 2.3548
        )
        params["sigma_2"].set(
        ...
        params["sigma_3"].set(
            value=estimate["fwhm"][2] / 2.3548, min=limits["fwhm"][2][0] / 2.3548, max=limits["fwhm"][2][1] / 2.3548
        )
        return params
    ...
    def estimate_dips(self, data, x):
        ...
        estimate["amplitude_3"].set(
            value=-estimate["amplitude_3"].value, min=-estimate["amplitude_3"].max, max=-estimate["amplitude_3"].min
        )
        return estimate
...
class GaussianLinear(FitModelBase):
    ...
    def estimate_peak(self, data, x):
        ...
        estimate = self.make_params()
        estimate["offset"].set(value=offset, min=min(data) - data_span / 2, max=max(data) + data_span / 2, vary=True)
        ...
        estimate["center"].set(
            value=gauss_fit["center"].value, min=gauss_fit["center"].min, max=gauss_fit["center"].max, vary=True
        )
        estimate["sigma"].set(
            value=gauss_fit["sigma"].value, min=gauss_fit["sigma"].min, max=gauss_fit["sigma"].max, vary=True
        )
        return estimate
...
class Gaussian2D(FitModelBase):
    ...
    def _model_function(x, offset, amplitude, center_x, center_y, sigma_x, sigma_y, theta):
        ...
        gauss = offset + amplitude * np.exp(-(a * x_prime**2 + 2 * b * x_prime * y_prime + c * y_prime**2))
        return gauss.ravel()
    ...
    def estimate_peak(self, data, x):
        ...
        estimate["center_x"].set(value=center_x, min=np.min(x[0]) - x_range / 2, max=np.max(x[0]) + x_range / 2)
        estimate["center_y"].set(value=center_y, min=np.min(x[1]) - y_range / 2, max=np.max(x[1]) + y_range / 2)
