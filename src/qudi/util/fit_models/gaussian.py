...
def multiple_gaussian(x, centers, sigmas, amplitudes):
    ...
    return sum(amp * np.exp(-((x - c) ** 2) / (2 * sig**2)) for c, sig, amp in zip(centers, sigmas, amplitudes))
...
class DoubleGaussian(FitModelBase):
    ...
    def estimate_peaks(self, data, x):
        ...
        leveled_data_smooth, offset = correct_offset_histogram(data_smoothed, bin_width=2 * filter_width)
