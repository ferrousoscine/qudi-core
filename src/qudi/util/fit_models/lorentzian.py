...
def multiple_lorentzian(x, centers, sigmas, amplitudes):
    ...
    return sum(amp * sig**2 / ((x - c) ** 2 + sig**2) for c, sig, amp in zip(centers, sigmas, amplitudes, strict=False))


def multiple_complex_lorentzian(
    x: float, centers: Sequence[float], sigmas: Sequence[float], amplitudes: Sequence[float], thetas: Sequence[float]
):
    ...
    length.
    """
    if not len(centers) == len(sigmas) == len(amplitudes) == len(thetas):
        raise ValueError("centers, sigmas, amplitudes, thetas must have same length")
    return sum(
        np.exp(1j * np.deg2rad(th)) * amp * (-1j * (x - c) * sig + sig**2) / (np.pi * sig * ((x - c) ** 2 + sig**2))
        for c, sig, amp, th in zip(centers, sigmas, amplitudes, thetas, strict=False)
    )
...
class DoubleLorentzian(FitModelBase):
    ...
    def _model_function(x, offset, center_1, center_2, sigma_1, sigma_2, amplitude_1, amplitude_2):
        return offset + multiple_lorentzian(x, (center_1, center_2), (sigma_1, sigma_2), (amplitude_1, amplitude_2))
