...
def multiple_poissonian(x, mus, amplitudes):
    ...
    """
    if len(mus) != len(amplitudes):
        raise ValueError("mus and amplitudes must have same length")
    ...
    if min(x) < 1e6:
        return sum(
            np.exp(xlogy(x, mu) - gammaln(x + 1) - mu) for mu, amp in zip(mus, amplitudes, amplitudes, strict=False)
        )
