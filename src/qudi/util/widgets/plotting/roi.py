...
class RectangleROI(ROI):
    ...
    def set_min_size(self, size: None | tuple[float, float]) -> None:
    ...
    def set_area(self, position: tuple[float, float] | None = None, size: tuple[float, float] | None = None) -> None:
        if position is not None:
            ...
            self.setPos(
                QtCore.QPointF(position[0] - constr_size[0] / 2, position[1] + constr_size[1] / 2),
                update=False,
                finish=False,
            )
        ...
        if size is not None:
            ...
            constr_size = (max(size[0], self.__min_norm_size[0]), max(size[1], self.__min_norm_size[1]))
            self.setSize(QtCore.QPointF(constr_size[0], -constr_size[1]), center=(0.5, 0.5), update=False, finish=False)
    ...
    def _clip_area(self, update: bool | None = True, finish: bool | None = True) -> None:
        ...
        translate = (position[0] - self.__center_position[0], position[1] - self.__center_position[1])
