...
def config_schema() -> dict[str, Any]:
    ...
    return {
        ...
        'properties': {
            'global': {
                ...
                'properties': {
                    'startup_modules': {
                        'type': 'array',
                        'uniqueItems': True,
                        'items': {'type': 'string', 'pattern': f'^{__module_name_pattern}$'},
                        'default': list(),
                    },
                    ...
                    'namespace_server_port': {'type': 'integer', 'minimum': 0, 'maximum': 65535, 'default': 18861},
                    'force_remote_calls_by_value': {'type': 'boolean', 'default': True},
                    'hide_manager_window': {'type': 'boolean', 'default': False},
                    'stylesheet': {'type': 'string', 'pattern': r'^[^.]+\.qss$', 'default': 'qdark.qss'},
            ...
            'logic': {
                ...
                'additionalProperties': {
                    'oneOf': [{'$ref': '#/$defs/local_module'}, {'$ref': '#/$defs/remote_module'}]
                },
                'default': dict(),
            },
            'hardware': {
        ...
        '$defs': {'local_module': local_module_config_schema(), 'remote_module': remote_module_config_schema()},
    }
...
def local_module_config_schema() -> dict[str, Any]:
    ...
    return {
        ...
        'additionalProperties': False,
        'properties': {
            'module.Class': {'type': 'string', 'pattern': f'^{__module_name_pattern}(\\.{__module_name_pattern})*$'},
            'allow_remote': {'type': 'boolean', 'default': False},
            'connect': {
                'type': 'object',
                'additionalProperties': {'type': 'string', 'pattern': f'^{__module_name_pattern}$'},
                'default': dict(),
            },
            'options': {'type': 'object', 'additionalProperties': True, 'default': dict()},
        },
    }
...
def remote_module_config_schema() -> dict[str, Any]:
    ...
    return {
        ...
        'additionalProperties': False,
        'properties': {
            'native_module_name': {'type': 'string', 'pattern': f'^{__module_name_pattern}$'},
            'address': {'type': 'string'},
            'port': {'type': 'integer', 'minimum': 0, 'maximum': 65535},
            'certfile': {'type': ['null', 'string'], 'default': None},
            'keyfile': {'type': ['null', 'string'], 'default': None},
